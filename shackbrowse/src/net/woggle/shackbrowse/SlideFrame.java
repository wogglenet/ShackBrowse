...
*/
    ...
    public SlideFrame(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Style
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SlideFrame);

        // Set the side of the screen
        setStickTo(ta.getInt(R.styleable.SlideFrame_stickTo, STICK_TO_AUTO));

        // Sets the shadow drawable
        int shadowRes = ta.getResourceId(R.styleable.SlideFrame_shadowDrawable, -1);
        if (shadowRes != -1) {
            setShadowDrawable(shadowRes);
        }

        // Sets the shadow width
        setShadowWidth((int) ta.getDimension(R.styleable.SlideFrame_shadowWidth, 0));

        // Sets the ability to close the layer by tapping in any empty space
        closeOnTapEnabled = ta.getBoolean(R.styleable.SlideFrame_closeOnTapEnabled, true);
    ...
    public void scrollToWrapper (int x, int y)
    {
    	...
    	if (getWidth() > 0)
    	{
	    	float granularity = 50f;
	    	float percentage = granularity * ((getScrollX() * -1f) / getWidth());
	    	percentage = Math.round(percentage);
	    	percentage = percentage / granularity;
	    	if (lastPercent != percentage)
	    	{
	    		lastPercent = percentage;
	    		mOnInteractListener.onScroll(lastPercent);
	    	}
    	}
    	*/
    }
...
*/
    ...
    private boolean allowDragging(float dx) {
        return mIsOpen && dx > 0;
    }
...
*/
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (w != oldw) {
            completeScroll();
            scrollToWrapper(getDestScrollX(), getScrollY());
        }
    }
